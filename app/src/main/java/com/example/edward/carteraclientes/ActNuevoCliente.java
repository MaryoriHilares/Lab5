        ...
        switch (id){
            ...
            case  R.id.action_eliminar:
                if(edtNombre.getText().toString().trim().isEmpty()){
                    AlertDialog.Builder dlg = new AlertDialog.Builder(this);
                    dlg.setTitle("Aviso");
                    dlg.setMessage("Complete el espacio de nombre para eliminar");
                    dlg.setNeutralButton("OK", null);
                    dlg.show();
                }
                else{
                ...
                conexion = datosOpenHelper.getWritableDatabase();
                StringBuilder sql = new StringBuilder();
                sql.append("DELETE FROM CLIENTE WHERE NOMBRE='"+edtNombre.getText().toString().trim()+"'");
                conexion.execSQL(sql.toString());
                conexion.close();
                //Toast.makeText(this,"Boton eliminar selecccionado",Toast.LENGTH_SHORT).show();
                finish();}
                break;
            case  R.id.action_buscar:
                //Toast.makeText(this,"Boton buscar selecccionado",Toast.LENGTH_SHORT).show();
                if(edtNombre.getText().toString().trim().isEmpty()){
                AlertDialog.Builder dlg = new AlertDialog.Builder(this);
                dlg.setTitle("Aviso");
                dlg.setMessage("Complete el espacio de nombre para buscar");
                dlg.setNeutralButton("OK", null);
                dlg.show();
                }
                else{
                ...
                sql.append("SELECT * FROM CLIENTE WHERE NOMBRE='"+edtNombre.getText().toString().trim()+"'");
                    edtDireccion.setText(resultado.getString(resultado.getColumnIndex("DIRECCION")));
                    edtEmail.setText(resultado.getString(resultado.getColumnIndex("EMAIL")));
                    edtTelefono.setText(resultado.getString(resultado.getColumnIndex("TELEFONO")));
                conexion.execSQL(sql.toString());
                conexion.close();
                finish();
                }
